...
public class AppUtilTest {
	...
	public void isGameAppTest1() {
		App app= new CardGameApp();   //CardGameのインスタンスをApp型のappに代入
		boolean result = AppUtil.isGameApp(app);   // 引数appをisGameAppメソッドに代入した際に、
		boolean expected = true;  //期待値と実際の値がイコール関係かを調べる
		
		assertEquals(expected, result);  
	//assertEquals(args1, args2): args1とargs2が同じ値か判別する
	}
	...
	//isCardGameAppメソッドのテストケース  //なんで毎回のテストケース全部のメソッドをテストするのか？
	@Test
    ...
    public void getAppNameTest1() {
    	...
    	assertEquals(result, expected);   //順番が逆でもOK
